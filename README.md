...
直接用瀏覽器打開 `index.html` 即可（建議 Chrome 或 Safari 14+，Firefox 對 Web Speech 支援差）。

注意：發音辨識需要 HTTPS（或 localhost），雙擊本地檔可能無法錄音。建議：

```bash
# Python 內建 server（任一即可）
...
- **發音準確度門檻寬鬆**：75% 大師、40% 老外能聽懂、15% 抓到重點。出國時敢開口比念得標準重要。
